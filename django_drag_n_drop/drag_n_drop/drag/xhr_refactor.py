...
class automator():
    def __init__(self, sprint_name, source_repo, target_repos, issue_array, base64_auth):
        ...
        self.headers = {'Accept': 'application/vnd.github.inertia-preview+json',
                        'Authorization': f'Basic {base64_auth.decode("utf-8")}'}
    ...
    def get_issues(self):
        ...
        self.source_issues = [issue(next(x for x in response if x['id'] == item['id']), item['priority']) for item in self.issue_array]
